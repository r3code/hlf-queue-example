...
var _ = Describe("HLFQueue", func() {
	...
	Describe("Items Rrordering :: MoveBefore", func() {
		It("Allows to move an item to the place BEFORE specified item in the middle", func() {
			// we need new empty queue
			ccMock3 := testcc.NewMockStub("hlfq_mock3", hlfq.New())
			expectcc.ResponseOk(ccMock3.From(Authority).Init()) // init chaincode2
			...
			movingItem := itemsInQueue[2] // Amount = 3
			beforeItem := itemsInQueue[1] // Amount = 2

			movedItem := expectcc.PayloadIs(
				ccMock3.From(Authority).Invoke("MoveBefore", movingItem.ID.String(), beforeItem.ID.String()),
				&hlfq.QueueItem{}).(hlfq.QueueItem)
		...
		It("Allows to move a tail item to the place Before head", func() {
			// we need new empty queue
			ccMock3 := testcc.NewMockStub("hlfq_mock3", hlfq.New())
			expectcc.ResponseOk(ccMock3.From(Authority).Init()) // init chaincode2
			...
			movingItem := itemsInQueue[1] // Amount = 2
			beforeItem := itemsInQueue[0] // Amount = 1
